...
- **목표 가격표만 프레임에 담아주세요.** Azure OCR은 초점이 안 맞고 작게 나온
  배경 텍스트까지도 읽어낼 정도로 성능이 좋아서, 뒤쪽 진열대의 다른 가격표가
  화면 구석에 흐릿하게라도 걸리면 그 텍스트까지 같이 인식되어 엉뚱한 상품 행이
  추가로 생길 수 있습니다(상품코드만 있고 이름/가격이 전혀 없는 조각은 자동으로
  걸러지지만, 이름/가격까지 읽히는 조각은 실제 다른 상품으로 처리됩니다).
