...
| `SHEET_NAME` | (기존 표시사항 라벨 탭 이름 - 하위호환용, 안 정해도 `표시사항라벨`로 자동 생성됨) |
...
가격표 탭 이름(`가격표-식품`, `가격표-비식품`)은 Secret 등록 없이도 자동으로 만들어지므로
꼭 필요하진 않지만, 다른 이름을 쓰고 싶으면 `SHEET_NAME_FOOD`, `SHEET_NAME_NONFOOD`도
같은 방식으로 등록하면 됩니다.
...
Drive 폴더에는 여러 종류의 사진을 섞어서 올려도 됩니다. 사진마다 구조적 특징으로
종류를 자동 판별해서, 같은 스프레드시트 안에 **탭 3개**로 나눠 저장합니다 (탭이
없으면 처음 실행할 때 자동으로 만듭니다 - 미리 만들어둘 필요 없음):

| 탭 이름 (기본값) | 판별 기준 | 컬럼 |
|---|---|---|
| `표시사항라벨` | "단가"/상품코드+가격 패턴이 없는 사진 (제품 뒷면 원재료 표시사항) | `파일ID`, `파일명`, `처리일시`, `제품명`, `식품유형`, `내용량`, `수입원 및 소재지`, `원산지 및 제조회사`, `소비기한`, `원재료명`, `보관방법`, `반품 및 교환장소`, `포장재질`, `알레르기정보`, `바코드`, `검토필요`, `원문텍스트` |
| `가격표-식품` | 매대 가격표이면서 중량 표기(`43G X 12` 등)가 있음 | `파일ID`, `파일명`, `처리일시`, `상품코드`, `제품명(한국어)`, `중량`, `제품명(영어)`, `단가`, `가격`, `검토필요`, `원문텍스트` |
| `가격표-비식품` | 매대 가격표이면서 중량 표기가 없음 | `파일ID`, `파일명`, `처리일시`, `상품코드`, `제품명(한국어)`, `제품명(영어)`, `단가`, `가격`, `검토필요`, `원문텍스트` |

탭 이름은 `SHEET_NAME_LABEL`, `SHEET_NAME_FOOD`, `SHEET_NAME_NONFOOD` 환경변수로
바꿀 수 있습니다 (안 정해두면 위 기본값 그대로 씀).

`파일ID`는 중복 처리 방지를 위한 식별자이며(세 탭을 모두 확인해서 판단합니다),
`원문텍스트`에는 항목 파싱과 무관하게 OCR로 읽은 전체 텍스트가 그대로 남아있어
파싱이 잘못된 경우 원문 대조가 가능합니다.
