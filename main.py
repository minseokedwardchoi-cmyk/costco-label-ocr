...
4. OCR 결과 텍스트에서 상품코드/제품명(한국어)/가격만 정규식으로 뽑는다
   (식품/비식품 구분 없이 항상 이 3개만 - 중량·영문명·단가 줄은 컬럼으로
   안 남기지만 이 3개의 경계를 찾는 데는 내부적으로 쓰인다).
...
# 식품/비식품 가릴 것 없이 사진 한 장(또는 카드 한 장)당 뽑아서 시트에 쓰는 건
# 이 4개 필드뿐이다. 중량/제품명(영어)/단가 등은 더 이상 컬럼으로 안 남기지만,
# 상품코드-제품명-가격의 위치를 정확히 찾아내는 데는 여전히 내부적으로 쓰인다
# (parse_price_fields 참고).
PRICE_FIELDS = ["상품코드", "제품명(한국어)", "가격"]
...
def parse_price_fields(text: str) -> dict:
    ...
    # 단가(예: "217원/10G")는 컬럼으로 안 남기지만, 가격 후보 중 단가 줄을
    # 제외하는 데는 필요하다 (같은 줄이 단가/가격 둘 다로 잘못 채택되는 것 방지).
    danga_price = ""
    danga_price_line_idx = None
